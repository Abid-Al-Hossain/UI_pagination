# UI Foundry Pagination Studio

Standalone visual Pagination generator built with Next.js, React, and TypeScript.

Standalone Pagination component studio for building native, preview-honest, React-exportable UI patterns that match the UI Foundry product line.
...
- designers building Pagination UI quickly
- developers who need editable React starting points
- marketplace buyers who want a self-contained Pagination generator
...
The editor is focused on controls that are native to Pagination and useful in real product work:
- page count, current page, sibling/boundary count, prev/next, first/last, and disabled state
- button/item size, gap, radius, color, border, hover, focus, active, and selected styling
- compact, full, dotted, mobile, and dashboard variants
- navigation landmarks and aria-current semantics
- React export for pagination UI

The goal is maximum useful depth without adding unrelated controls that would make the component stop reading as Pagination.
...
This repo is intended to publish as Abid-Al-Hossain/UI_pagination.
